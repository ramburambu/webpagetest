  ...
  if( isset($testPath) ) {
    ...
    if (isset($options["run"]) && $options["run"]) {
      ...
      if( isset($options['cached']) ) {
        $pageData[$run][$options['cached']] = loadPageRunData($testPath, $run, $options['cached'], $multistep);
        if (!isset($pageData[$run][$options['cached']]))
          unset($pageData);
      } else {
        $pageData[$run][0] = loadPageRunData($testPath, $run, 0, $multistep);
        if (!isset($pageData[$run][0]))
          unset($pageData);
        $pageData[$run][1] = loadPageRunData($testPath, $run, 1, $multistep);
      }
    }
